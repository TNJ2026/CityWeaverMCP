...
* **最大分区进深**：沿市政道路向两侧法线方向延伸极限为 **6 个单元格（48.0 m）**。
* **黄金街区宽度（Golden Block Width）**：
  * 对开道路的最佳进深为 **96.0 m（48 m × 2 = 12 cells）**。
  * **物理特性**：两侧道路产生的分区在中心线严丝合缝背靠背对接，**空间利用率 100%**，零死角零缝隙。
  * 若间距 > 96 m：中心会残留无法分区的狭缝（虚度地皮）；
  * 若间距 < 96 m：两侧分区深度受压迫，单侧建筑进深被削减。
...
* **平整度安全裕度**：规划网格组团时，要求地表高程极差 ΔY_max ≤ Length × 0.10。若落差过大，需先调用整地工具或梯级放坡。
