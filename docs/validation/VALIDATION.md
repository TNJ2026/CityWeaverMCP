...
- **地形与地块边界**：初始拥有 9 块标准地图格（3×3 矩阵，X ∈ [-1558.26, 311.65]，Z ∈ [-311.65, 1558.26]），地表平缓，标高在 592 m ~ 606 m 之间。
- **动脉与微循环骨架**：
  - 东西向 4 车道大道（`Medium Road`）：轴线 Z = 546.5，从高速接入端 (-123.2, 545.5) → (-283.2, 546.0) → (-443.2, 546.5) 一路向西贯穿至 (-1350.0, 546.5)。
  - 南北向 4 车道大道（`Medium Road`）：轴线 X = -750.0，贯通 Z ∈ [150.0, 950.0]。
  - 内部街区路网（`Small Road`）：
    - 南苑低密生活区（Zone 1）：X ∈ [-1150, -750]，Z ∈ [150, 546.5]，4 大方正街区。
    - 北苑低密生活区（Zone 2）：X ∈ [-1150, -750]，Z ∈ [546.5, 950]，4 大方正街区。
    - 西区中密生活城（Zone 3）：X ∈ [-1350, -1150]，Z ∈ [350, 750]，大开间联排生活区。
    - 东区迎宾商业区（Zone 4）：X ∈ [-600, -443.2]，Z ∈ [350, 546.5]。
    - 中枢核心商业区（Zone 5）：X ∈ [-600, -443.2]，Z ∈ [546.5, 750]。
- **生命线公用工程**：
  ...
  - 变电站 `TransformerStation01`：移至北部独立专用支路 (-786.25, 592.38, 1250.0)，距住宅区 > 300 m，通过地下高压电缆 `High-voltage Ground Cable` 直连北部高压线节点 (-742.6, 1469.1)。
- **自然晋级里程碑与市政服务实测落地**：
  - **Milestone 1（750 XP）**：落成社区诊所 `MedicalClinic02`（X = -782.25，Z = 646.5，成本 ₡30,000）与清幽公墓 `Cemetery02`（X = -717.75，Z = 1066.0，成本 ₡42,000）。
  - **Milestone 2（2,500 XP）**：解锁并施划 `EU Residential Medium Row`（Zone 3: 734 户联排全部满员入住）；落成南苑小学与北苑小学 `ElementarySchool02` ×2（分别位于 X = -713.75, Z = 450.0 与 X = -410.93, Z = 750.0）。
...
- **低密度工业园规划实测（东部高速北翼工业园）**：
  ...
  - 园区范围：X ∈ [-283.18, -120.0]，Z ∈ [650.0, 880.0]，废气完全顺风排向东北无人区，与生活区保持 > 460 m ~ 850 m 隔离；
  - 道路：由高速入口端点 (-283.18, 546.0) 引出 4 车道 `Medium Road`（长 204 m）直通工业园，内部闭合 2 组方正工业支路街区（`Small Road`）；
