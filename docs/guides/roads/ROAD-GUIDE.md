...
25. `preview_road_policies` 原子修改道路名及选定汽车车道的限速、公交专用和转向旗标。提交前再次比对预览快照；任一目标变化会拒绝整批提交，写入中出错会回滚已写项目。路边停车使用 `preview_road_parking`。
...
2. **路网防平行干涉与安全间距控制**：
   - 当在既有路网边缘新建平行干道或高速接驳线时，两条平行道路中心线的横向间距必须 $\ge 32\text{m}\sim 48\text{m}$（绝对底线不能小于两条道路半宽之和加 8m）。
   - 过近的平行道路会触发原生路基物理重叠冲突（`Overlap`），或造成两侧 8m 分区单元格（Zone Cells）相互争夺、截断破坏，导致临街建筑无法生成。若需要伴行新建道路，优先调用 `preview_road_parallel` 并设置合理的 `offset_m` 及 `avoid_obstacles=true`。
