...
单段长度的四个口径（引擎 256 m 上限 / 自动路线输出 248 m / 脚本等分安全线 240 m / 几何质量建议 200 m）角色不同、不可互相替代，见[游戏物理规则](../../reference/GAME-PHYSICS-RULES.md) §1.4。
...
27. `preview_road_undo` 对同一城市会话内已完成的独立新路、拆除、反向、同源预制件升级和直接策略生成精确逆操作。拆分既有道路的创建操作会被安全拒绝自动撤销，因为游戏的完成集合包含既有道路替换片段；这类操作应按返回道路类型和目标显式拆除。
28. `preview_road_elevation` 输入 1–64 个永久道路 `edge_ids` 和 `clearance_m`，按实时地形高度整体移动道路中心线。它保留道路的平面曲线、预制件、边实体及共享节点拓扑；提交时原子更新所有唯一节点、四点贝塞尔曲线和高程组件，并触发游戏重建道路几何与车道。预览后任一节点变化会拒绝提交，写入异常会恢复原节点、曲线和高程。地面道路完成更新后，游戏可能自动把紧邻地形贴合到道路基底；高架桥仍应使用新建道路的 8 米以上 `elevation_m`。
...
规划道路、网格出口、建筑支路或外部接驳线时，候选接入点必须按以下顺序选择；不得仅因直线距离更短就跳过更高优先级候选：

1. **已有路口节点**：优先复用目标道路上已经连接多条道路的永久路口节点。端点使用实时发现的 `node_id`，避免在其附近再造一个过密的新路口。
2. **已有转角或端点节点**：没有合适路口时，优先接入现有道路的转角、T 形预留端点、断头路端点或规划包络角点上的永久节点。连接方向应尽量延续既有切线或形成清晰的 90° 转角，避免短折线和锐角。
3. **没有路口的道路中段**：只有前两类候选不存在、超出允许接驳范围，或因坡度、净空、碰撞、道路方向与容量而无法通过原生预览时，才允许用 `edge_id + edge_position` 精确拆分道路中段生成新路口。中段接入必须检查与两端既有路口的退距、转向排队空间、分区格破坏和主干道接入口密度。

“优先”不等于强行连接：若较高优先级候选会越出已购区域、穿越建筑或水域、形成不允许的坡度/锐角，或把本地道路直接接入不适合的高速及主干路，可记录拒绝原因后评估下一优先级。无论选择哪一级，都必须经过原生道路 preview；提交后回读永久节点及相邻道路，确认实际形成连接而不是图面相交。

规则网格还必须保持内部道路等宽：内部横纵道路优先使用同一 prefab，至少不得混用实际宽度不同的道路；网格内部禁止扩宽、收窄或设置宽度过渡。更宽道路只能用于网格外围，或从外围节点向外连接城市道路的接驳/集散路。宽度变化应位于网格边界节点或外围路口，并通过原生 preview 检查路口几何、净空、车道衔接和分区格影响。
