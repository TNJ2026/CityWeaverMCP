...
1. **排污口管道单向物理隔离（防污水倒灌）**：
   - 排污设施（如 `SewageOutlet01`）仅需向市政路网排放污水。向市政道路接驳时，必须严格使用专用纯污水管（`Small Sewage Pipe`）。
   - **严禁混接**：绝对禁止在排污口到道路之间敷设供排水合流管（`Combined Small Pipe`）或供水管。若使用合流管混接排污口，排污管网的污水会逆向渗入市政净水管路，导致全城自来水瞬间重度污染，引发全城性市民水污染中毒危机。
   - 排污口自身不需要接入任何自来水给水管。
2. **变电站高压安全岛模式（Noise & Safety Isolation）**：
   - 变电站（`TransformerStation01`）具备较强的电磁噪音和负面属性，严禁紧贴住宅区边缘放置。
   - **专用支路**：变电站应选址于靠近外部高压走廊的独立地块，单独修建一条 32m~64m 的专用本地街道（`Small Road`）将其连入城市路网，道路内嵌的低压配电网会自动覆盖变电站。
   - **高压电缆接驳**：从外部高压输电塔（`PowerLine01`）节点或高压线边，敷设地下高压电缆（`High-voltage Ground Cable`，高程通常在 -10m 至 -15m）直连变电站高压接口，形成物理隔绝的安全受电岛。
