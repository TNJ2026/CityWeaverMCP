...
规划重污染或通用工业区（如 `Industrial Manufacturing`）时，必须遵守以下环境选址准则：

1. **风向矢量判定**：在选址前必须先调用 `get_climate_state`，读取当前气候环境的恒定风向矢量（如 `wind: { x, z }`）。
2. **绝对下风向选址**：工业区必须设置在居民区、商业区及医院/学校等敏感公共设施的**正下风侧边缘**（即沿着风向矢量背离生活区的方位）。确保工业烟尘、废气直接吹向地图未开发边界，杜绝向生活区倒灌。
3. **水源与地下水绝对安全距离**：工业区选址必须远离饮用水水源地、地表水取水口、地下水富集区以及水塔。与净水设施必须保持至少 500m 以上的空间隔离，且严禁置于地下水流向居民区的上游径流区。
4. **污染缓冲区（Buffer Zone）**：工业区边界与最近的居住区之间应保留至少 300m～500m 的缓冲带，缓冲带内可布置林木绿化（利用 `place_landscape_objects`）、高速公路走廊或低污染物流仓储设施，阻隔空气污染与重工业噪音扩散。
