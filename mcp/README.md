...
新增高层编排：`deploy_service_cluster`、`deploy_industrial_campus`、`deploy_transit_corridor`。三者按阶段串行复用原生 preview/apply，使用稳定 `request_id`，遇到失败或 `outcome_unknown` 停止后续阶段，不提供跨领域自动回滚。
