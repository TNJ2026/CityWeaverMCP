...
namespace CityWeaver
{
    ...
    public sealed partial class GameQueryService
    {
        ...
        {
            string key=RequestKey(args),fingerprint=type+":"+args.ToString(Formatting.None);if(m_DistrictRequestIds.TryGetValue(key,out var id)){var old=m_DistrictOperations[id];if(old.Fingerprint!=fingerprint)throw new QueryException("REQUEST_ID_CONFLICT","request_id was already used with different district arguments.");return old;}if(m_DistrictOperations.Count>=4096)throw new QueryException("TOO_MANY_OPERATIONS","This city session already contains 4096 district operations.");
