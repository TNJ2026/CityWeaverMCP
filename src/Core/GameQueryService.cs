...
namespace CityWeaver
{
    ...
    public sealed partial class GameQueryService : IDisposable
    {
        ...
        public JObject Execute(JObject request)
        {
            ...
            if (tool == "get_entity_components" || tool == "query_entities" || tool == "find_roads_by_name" || tool == "find_buildings_by_name" || tool == "get_city_data" || tool == "inspect_road_zoning" || tool == "analyze_zoning_cells" || tool == "get_planning_map_snapshot") SyncReads(world);
            switch (tool)
            {
                ...
                case "find_buildings_by_name": return Wrap(FindBuildingsByName(args, world, em));
                ...
                case "list_intersection_prefabs": return Wrap(ListIntersectionPrefabs(args, world));
                case "preview_intersection_prefab": return Wrap(PreviewIntersectionPrefab(args, world));
                ...
                case "list_road_stop_prefabs": return Wrap(ListRoadStopPrefabs(args, world));
                case "plan_road_stop_site": return Wrap(PlanRoadStopSite(args, world));
                case "preview_road_stop_placement": return Wrap(PreviewRoadStopPlacement(args, world));
                case "get_road_stop_operation": return Wrap(GetRoadStopOperation(args));
                case "apply_road_stop_operation": return Wrap(ApplyRoadStopOperation(args, world));
                case "cancel_road_stop_preview": return Wrap(CancelRoadStopPreview(args));
                ...
                case "list_waterway_prefabs": return Wrap(ListWaterwayPrefabs(args, world));
                case "list_waterways": return Wrap(ListWaterways(args, world));
                case "get_waterway": return Wrap(GetWaterway(args, world));
                case "preview_waterway": return Wrap(PreviewWaterway(args, world));
                case "preview_waterway_delete": return Wrap(PreviewWaterwayDelete(args, world));
        ...
        private JObject Capabilities()
        {
            ...
            ["road_building"] = new JObject { ["modes"] = new JArray("straight", "quadratic", "cubic", "elevated", "tunnel", "polyline"), ["min_length_m"] = 16, ["max_length_m"] = 256,
                ["requires_paused_city"] = true, ["preview_ttl_seconds"] = 300, ["max_operations_per_session"] = 4096,
